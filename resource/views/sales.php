...
// Product query for add form (include price)
$product_query = "SELECT p.product_id, p.product_name, p.price, i.stock_quantity 
                 FROM Product p 
            ...
            <div id="product-entries">
                <div class="product-entry">
                    <select class="form-control" name="product_id[]" required
                        onchange="updateQuantityLimitAndPrice(this)">
                        ...
                        <?php foreach ($products as $product): ?>
                            <option value="<?= $product['product_id'] ?>" data-quantity="<?= $product['stock_quantity'] ?>"
                                ...
                                <?= $product['stock_quantity'] ?>, Price: ₱<?= number_format($product['price'], 2) ?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <input class="form-control" type="number" name="quantity[]" placeholder="Quantity" required min="1"
                        max="">
                    <input class="form-control" type="number" step="0.01" name="unit_price[]" readonly>
...
<!-- VERIFY SALES MODAL -->
<div class="modal fade" id="verifySalesModal" tabindex="-1" aria-labelledby="verifySalesModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="verifySalesModalLabel">Verify Sale Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <label>Sale Date:</label>
                <p id="verify_sale_date"></p>
                <label>Customer ID:</label>
                <p id="verify_customer_id"></p>
                <label>Payment Method:</label>
                <p id="verify_payment_method"></p>
                ...
                <table class="table table-striped">
                    ...
                    <tbody id="verify_products"></tbody>
                </table>
                <label>Total Amount (₱):</label>
                <p id="verify_total_amount"></p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-danger" data-bs-dismiss="modal"
                    onclick="cancelVerification()">Cancel</button>
                <button type="button" class="btn btn-save" onclick="confirmSale()">Save Sale</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        document.querySelectorAll('select[name="product_id[]"]').forEach(select => updateQuantityLimitAndPrice(
            select));
    });
    ...
    window.addProductEntry = function() {
        ...
        entry.innerHTML = `
            <select class="form-control" name="product_id[]" required onchange="updateQuantityLimitAndPrice(this)">
                <option value="">Select Product</option>
                ${<?php echo json_encode($products); ?>.map(product => 
                    `<option value="${product.product_id}" data-quantity="${product.stock_quantity}" data-price="${product.price}">
                        ${product.product_name} (ID: ${product.product_id}, Stock: ${product.stock_quantity}, Price: ₱${Number(product.price).toFixed(2)})
                    </option>`).join('')}
            </select>
    ...
    // Update quantity limit and price
    window.updateQuantityLimitAndPrice = function(selectElement) {
    ...
    let formDataToSave = null;
    document.getElementById('addSaleForm').addEventListener('submit', function(e) {
        e.preventDefault();
        const formData = new FormData(this);
        formDataToSave = formData; // Store form data for later use

        // Extract form values
        const customerId = formData.get('customer_id') || 'N/A';
        ...
        // Calculate totals and populate verification modal
        let totalAmount = 0;
        const tbody = document.getElementById('verify_products');
        tbody.innerHTML = '';
        const productsList = <?php echo json_encode($products); ?>;

        for (let i = 0; i < productIds.length; i++) {
            const product = productsList.find(p => p.product_id === productIds[i]);
            const productName = product ? product.product_name : 'Unknown';
            const quantity = parseInt(quantities[i]) || 0;
            const unitPrice = parseFloat(unitPrices[i]) || 0;
        ...
        document.getElementById('verify_sale_date').textContent = saleDate;
        document.getElementById('verify_customer_id').textContent = customerId;
        document.getElementById('verify_payment_method').textContent = paymentMethod;
        document.getElementById('verify_total_amount').textContent = totalAmount.toFixed(2);

        // Show verification modal
        const verifyModal = new bootstrap.Modal(document.getElementById('verifySalesModal'));
        verifyModal.show();
    });

    // Confirm sale and save to server
    window.confirmSale = function() {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    data.deductions.forEach(d => {
                        deductionMessage += `Product ID ${d.product_id}: ${d.quantity_deducted} unit(s), `;
                    });
        ...
        const verifyModal = bootstrap.Modal.getInstance(document.getElementById('verifySalesModal'));
        verifyModal.hide();
    };

    // Cancel verification
    window.cancelVerification = function() {
        formDataToSave = null; // Clear stored form data
    };
    ...
    window.loadViewModal = function(sale) {
        ...
        document.getElementById('view_total_amount').textContent = sale.total_amount !== null && sale.total_amount !==
            undefined ? Number(sale.total_amount).toFixed(2) : '-';

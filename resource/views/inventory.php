...
if (isset($_GET['search']) && $_GET['search'] === '1') {
    ...
    if (isset($_GET['product_id']) && !empty($_GET['product_id'])) {
        $whereConditions[] = "i.product_id = '" . $conn->real_escape_string($_GET['product_id']) . "'";
    }
...
<div class="main-content">
    ...
    <div class="products-table">
        <div class="table-controls">
            <select id="stockLevelFilter" class="form-control" style="padding: 8px; border-radius: 5px;"
            ...
            <select id="orderBy" class="form-control" style="padding: 8px; border-radius: 5px;"
                onchange="applyTableFilters()">
                <option value="inventory_id|ASC">ID (Ascending)</option>
                <option value="inventory_id|DESC">ID (Descending)</option>
                ...
                <option value="received_date|DESC">Received Date (Newest)</option>
                <option value="received_date|ASC">Received Date (Oldest)</option>
            </select>
...
function searchInventory() {
    ...
    // Corrected path to web-layout.php
...
function applyTableFilters() {
    ...
    if (inventoryId || productId || price || stock) params.push('search=1');
    if (inventoryId) params.push(`inventory_id=${encodeURIComponent(inventoryId)}`);
    if (productId) params.push(`product_id=${encodeURIComponent(productId)}`);
    if (price) params.push(`price=${encodeURIComponent(price)}`);
    if (stock) params.push(`stock=${encodeURIComponent(stock)}`);

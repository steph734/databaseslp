...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['return_id']) && !empty($_POST['return_id'])) {
    $return_ids = $_POST['return_id']; // Fix: Use directly as it's an array

    try {
        $conn->begin_transaction();

        // Prepare delete query dynamically
        $query = "DELETE FROM supplierreturn WHERE supplier_return_id IN (" . implode(',', array_fill(0, count($return_ids), '?')) . ")";
        ...
        if ($delete_stmt->execute()) {
            if ($delete_stmt->affected_rows > 0) {
                $conn->commit();
                $_SESSION['success'] = "Supplier returns deleted successfully!";
            } else {
                $conn->rollback();
